...
class MockLock:
    ...
    def acquire(self, blocking: bool = True, **kwargs) -> bool:
        if not self.locked:
            self.locked = True
...
def app_instance() -> "Generator['Pynenc', None, None]":
    ...
    def mock_lock_factory(name: str, *args, **kwargs) -> MockLock:
    ...
    with patch.object(mongo_client, "get_redis_client", return_value=fakeredis.FakeRedis()), \
